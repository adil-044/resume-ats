  ...
  if (!analysisResult) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-black gap-10">
        ...
        <p className="text-white font-black uppercase tracking-[0.5em] text-[10px] animate-pulse italic">Initializing_Suite...</p>
      </div>
    );
  }
        ...
        {showPaywall && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-black border border-white/10 rounded-[3rem] p-16 max-w-xl text-center shadow-2xl overflow-hidden border-beam">
              ...
              <h2 className="text-5xl font-black text-white tracking-tighter uppercase leading-none mb-6 italic">Professional <br /> Tier Required.</h2>
              <p className="text-lg text-slate-500 font-medium mb-12 uppercase tracking-tighter italic">AI Re-engineering is reserved for authenticated professional nodes.</p>
              <div className="space-y-4">
                <button className="w-full py-6 bg-white text-black rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all active:scale-95 shadow-2xl">Upgrade Now</button>
                <button onClick={() => setShowPaywall(false)} className="w-full py-4 text-slate-600 font-black text-[9px] uppercase tracking-[0.4em] hover:text-white transition-colors">Return to Interface</button>
              </div>
            </motion.div>
          </div>
        )}
      ...
      <header className="bg-black border-b border-white/5 px-8 py-4 flex items-center justify-between z-30 shadow-2xl">
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <span className="text-[8px] font-black text-slate-700 uppercase tracking-[0.4em] leading-none mb-2 italic">Identity_Stream</span>
            <div className="flex items-center gap-3">
              <Terminal className="h-4 w-4 text-indigo-500/50" />
              <span className="font-black text-white text-sm tracking-tight italic uppercase truncate max-w-[200px]">{analysisResult.job_title || 'Untitled'}.MD</span>
            </div>
          </div>
        </div>
        
        <div className="hidden md:flex items-center gap-2 bg-white/5 p-1.5 rounded-2xl border border-white/10 shadow-inner">
          {[{ id: 'editor', icon: <Edit3 className="h-3.5 w-3.5" />, label: 'Write' }, { id: 'split', icon: <SplitSquareVertical className="h-3.5 w-3.5" />, label: 'Quantum' }, { id: 'preview', icon: <Eye className="h-3.5 w-3.5" />, label: 'Visual' }].map((pane) => (
        ...
        <div className="flex items-center gap-4">
          <button onClick={handleAIRephrase} disabled={isOptimizing} className="group flex items-center gap-3 px-8 py-3 bg-white/5 text-white border border-white/10 rounded-2xl font-black text-[9px] uppercase tracking-widest hover:bg-indigo-600 transition-all active:scale-95">
            {isOptimizing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4 text-indigo-400 group-hover:rotate-12 transition-transform" />} AI_Optimize
          </button>
          <button onClick={async () => { try { await exportResume(markdown); } catch (e) { alert('Export failed'); } }} className="flex items-center gap-3 px-8 py-3 bg-indigo-600 text-white rounded-2xl font-black text-[9px] uppercase tracking-widest hover:bg-white hover:text-black transition-all shadow-2xl active:scale-95">
            <Download className="h-4 w-4" /> Export_PDF
          </button>
        </div>
      </header>
        ...
        <aside className="w-80 bg-black border-r border-white/5 overflow-y-auto flex flex-col intelligence-scrollbar z-20 hidden lg:flex">
          <div className="p-8 space-y-12">
            <div className="space-y-8">
              <div className="flex justify-between items-end px-2"><h3 className="text-[8px] font-black text-slate-700 uppercase tracking-[0.5em] italic">Extraction</h3><span className="text-[8px] font-black text-slate-800 uppercase">Base: {analysisResult.initial_score}%</span></div>
              <div className="space-y-4">
                <div className="flex flex-col"><span className="text-[8px] font-black text-indigo-500/60 uppercase tracking-[0.4em] mb-2 italic">Optimized_Signal</span><span className="text-6xl font-black text-white tracking-tighter drop-shadow-2xl">{analysisResult.overall_score}<span className="text-xl opacity-20">%</span></span></div>
                <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden p-0.5 border border-white/5 shadow-inner"><motion.div initial={{ width: 0 }} animate={{ width: `${analysisResult.overall_score}%` }} className="h-full bg-indigo-600 rounded-full shadow-[0_0_10px_rgba(79,70,229,1)]" /></div>
              </div>
              <div className="grid grid-cols-1 gap-2">
                {Object.entries(analysisResult.breakdown).map(([key, val]) => (
                  <div key={key} className="bg-white/5 border border-white/5 px-5 py-3 rounded-2xl flex justify-between items-center hover:bg-indigo-600/5 transition-all"><p className="text-[8px] font-black text-slate-600 uppercase tracking-widest italic">{key.replace('_', ' ')}</p><p className="text-[10px] font-black text-white">{val}%</p></div>
                ))}
              </div>
            </div>

            <div className="space-y-8">
              <div className="flex justify-between items-center px-2"><h3 className="text-[8px] font-black text-white uppercase tracking-[0.5em] flex items-center gap-3 italic"><Terminal className="h-4 w-4 text-indigo-500" /> Registry</h3>{analysisResult.missing_keywords.length > 0 && <button onClick={() => setShowPaywall(true)} className="text-[8px] font-black text-indigo-400 uppercase tracking-widest bg-indigo-500/10 px-3 py-1.5 rounded-xl hover:bg-white hover:text-black transition-all shadow-xl active:scale-95"><Zap className="h-3 w-3" /> Bridge</button>}</div>
              <div className="space-y-2">
                {analysisResult.matched_keywords?.map((kw, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-green-500/5 border border-green-500/10 rounded-2xl"><span className="text-[10px] font-black text-green-400 uppercase tracking-tighter">{kw}</span><CheckCircle2 className="h-3 w-3 text-green-500" /></div>
                ))}
                <div className="relative group">
                  <div className="space-y-2 filter blur-[2px] opacity-20 select-none">
                    {analysisResult.missing_keywords.slice(0, 2).map((kw, i) => (
                      <div key={i} className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-2xl"><span className="text-[10px] font-black text-slate-600 uppercase tracking-tighter">{kw}</span><XCircle className="h-3 w-3 text-slate-800" /></div>
                    ))}
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center z-10"><button onClick={() => setShowPaywall(true)} className="px-4 py-2 bg-white text-black rounded-xl font-black text-[8px] uppercase tracking-[0.3em] shadow-2xl">Unlock Delta</button></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <main className="flex-1 flex overflow-hidden relative z-10">
          {(activePane === 'editor' || activePane === 'split') && (
            <div className={`flex-1 flex flex-col bg-black ${activePane === 'split' ? 'border-r border-white/5' : ''} z-10`}>
              <textarea value={markdown} onChange={(e) => setMarkdown(e.target.value)} className="flex-1 p-12 font-mono text-[14px] leading-[2.2] outline-none resize-none text-slate-400 selection:bg-indigo-500/40 placeholder:text-slate-900 bg-transparent intelligence-scrollbar" spellCheck={false} placeholder="// SYSTEM_INITIALIZED..." />
            </div>
          )}
